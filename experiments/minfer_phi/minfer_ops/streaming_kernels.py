...
class StreamingForward(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, n_init, n_local):
        # Output shape: (bsz, num_heads, seq_len, head_dim)
        ctx.save_for_backward(q, k, v, n_init, n_local)
        return streaming_forward_(q, k, v, n_init, n_local)
    
    @staticmethod
    def backward(ctx, grad_output):
        # grad_output shape: (bsz, num_heads, seq_len, head_dim)
        return grad_output.clone(), grad_output.clone(), grad_output.clone(), None, None
    
def streaming_forward(q, k, v, n_init, n_local):
    return StreamingForward.apply(q, k, v, n_init, n_local)

register_op('b num_head^ l^ hd^, b num_head^ l^ hd^, b num_head^ l^ hd^ -> b num_head^ l^ hd^')(streaming_forward)